...
def splitBreaks(string, periodsToBreaks):
    if (not periodsToBreaks):
        ...
        lastChar1 = None
        lastChar2 = None
        breakChars = map(chr, xrange(ord(''), ord('z') + 1))
        for c in string:
...
def kwic(string, ignoreWords=[], listPairs=False, periodsToBreaks=False):
    ...
    if (not listPairs):
        ...
        return (sorted(filteredLines, key=lambda l: (map(lambda w: w.lower(), l[0]), l[1])), map(lambda wp: (wp, pairs[wp]), sorted(filter(lambda wp: (pairs[wp] > 1), pairs.keys()))))